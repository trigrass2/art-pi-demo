...
int ftp_process_request(struct ftp_session* session, char *buf)
{
	...
	//
	//-----------------------
	if(str_begin_with_m(buf, "USER")==0)	
	{
	...
	else if(str_begin_with_m(buf, "PASS")==0)
	{
	...
	else if(str_begin_with_m(buf, "LIST")==0  )
	{
	...
	else if(str_begin_with_m(buf, "NLST")==0 )
	{
		memset(sbuf, 0, FTP_BUFFER_SIZE);
		rt_sprintf(sbuf, "150 Opening Binary mode connection for file list.\r\n");
		send(session->sockfd, sbuf, strlen(sbuf), 0);
	...
	else if(str_begin_with_m(buf, "PWD")==0 || str_begin_with_m(buf, "XPWD")==0)
	{
		rt_sprintf(sbuf, "257 \"%s\" is current directory.\r\n", session->currentdir);
		send(session->sockfd, sbuf, strlen(sbuf), 0);
	}
	else if(str_begin_with_m(buf, "TYPE")==0)
	{
		// Ignore it
	...
	else if(str_begin_with_m(buf, "PASV")==0)
	{
		int dig1, dig2;
		int sockfd;
		char optval='1';
...
err1:
	...
	else if (str_begin_with_m(buf, "RETR")==0)
	{
		int file_size;
	...
	else if (str_begin_with_m(buf, "STOR")==0)
	{
	...
	else if(str_begin_with_m(buf, "SIZE")==0)
	{
		int file_size;
	...
	else if(str_begin_with_m(buf, "MDTM")==0)
	{
		rt_sprintf(sbuf, "550 \"/\" : not a regular file\r\n");
		send(session->sockfd, sbuf, strlen(sbuf), 0);
	}
	else if(str_begin_with_m(buf, "SYST")==0)
	{
		rt_sprintf(sbuf, "215 %s\r\n", "RT-Thread RTOS");
		send(session->sockfd, sbuf, strlen(sbuf), 0);
	}
	else if(str_begin_with_m(buf, "CWD")==0)
	{
		build_full_path(session, parameter_ptr, filename, 256);
        DIR *dirp = opendir(filename);
        if(dirp == RT_NULL)
        {
            rt_sprintf(sbuf, "550 \"%s\" : dir is not exist\r\n", filename);
            send(session->sockfd, sbuf, strlen(sbuf), 0);
            rt_free(sbuf);
            return 0;
        }
	...
	else if(str_begin_with_m(buf, "CDUP")==0)
	{
	...
	else if(str_begin_with_m(buf, "PORT")==0)
	{
		int i;
		int portcom[6];
		char tmpip[100];
	...
	else if(str_begin_with_m(buf, "REST")==0)
	{
	...
	else if(str_begin_with_m(buf, "MKD")==0)
	{
		if (session->is_anonymous == RT_TRUE)
		{
			rt_sprintf(sbuf, "550 Permission denied.\r\n");
	...
	else if(str_begin_with_m(buf, "DELE")==0)
	{
		if (session->is_anonymous == RT_TRUE)
		{
			rt_sprintf(sbuf, "550 Permission denied.\r\n");
	...
	else if(str_begin_with_m(buf, "RMD")==0)
	{
		if (session->is_anonymous == RT_TRUE)
		{
			rt_sprintf(sbuf, "550 Permission denied.\r\n");
	...
	else if(str_begin_with_m(buf, "QUIT")==0)
	{
...
int ftp_init(void)
{
	rt_thread_t tid = rt_thread_create("ftpd", ftpd_thread_entry, RT_NULL, 4096, 27, 100);
	if(tid != RT_NULL)
